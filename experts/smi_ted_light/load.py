    ...
    def encode(self, smiles, useCuda=False, batch_size=100, return_torch=False):
        ...
        embeddings = [
            self.extract_embeddings(list(batch))[2].cpu().detach().numpy() 
                for batch in np.array_split(smiles, n_split)
        ]
...
def load_smi_ted(local_dir="experts/smi_ted_light"):
    vocab_path = os.path.join(local_dir, "bert_vocab_curated.txt")
    ckpt_path = os.path.join(local_dir, "smi-ted-Light_40.pt")
